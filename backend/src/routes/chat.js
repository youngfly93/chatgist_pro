    ...
    if (stream) {
      ...
      try {
        ...
        if (jsonMatch) {
          ...
          // 2. HTML注释格式（新格式，隐藏JSON）- 支持不完整的标签格式
          let hiddenMatch = reply.match(/<!--PHOSPHO_ANALYSIS_START-->\s*([\s\S]*?)\s*<!--PHOSPHO_ANALYSIS_END-->/);
          if (!hiddenMatch) {
            // 兼容AI生成的不完整标签格式（缺少结束的-->）
            hiddenMatch = reply.match(/<!--PHOSPHO_ANALYSIS_START>\s*([\s\S]*?)\s*<!--PHOSPHO_ANALYSIS_END-->/);
          }
          if (hiddenMatch) {
