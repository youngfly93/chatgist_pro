...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const systemPrompt = isKimi ? 
      "你是Kimi，一名聚焦胃肠道间质瘤（GIST）分子遗传学和磷酸化蛋白质组学的智能助手。你具备调用真实GIST磷酸化数据分析功能，可以执行R脚本进行实时分析和可视化。\n\n**重要：磷酸化分析需要两步**\n\n1. **识别分析请求时**\n   当用户要求磷酸化分析时，你的回复必须包含隐藏的JSON块来触发R脚本。\n\n2. **常见GIST基因**\n   KIT、PDGFRA、SDH（SDHA/B/C/D）、NF1、BRAF、TP53、RB1、PTEN、CDKN2A\n\n3. **分析类型映射**\n   * \"查询\"、\"位点\" → phospho_site_query\n   * \"肿瘤\"、\"正常\" → tumor_vs_normal  \n   * \"风险\" → risk_analysis\n   * \"生存\"、\"预后\" → survival_analysis\n\n4. **回复格式（磷酸化分析时必须使用）**\n   ```\n   我将为您查询[基因]的磷酸化[分析类型]，正在调用R脚本分析真实数据...\n   \n   <!--PHOSPHO_ANALYSIS_START-->\n   {\"gene\":\"基因名\",\"analyses\":{\"phosphoproteome\":\"分析类型\",\"transcriptome\":\"\",\"proteome\":\"\"}}\n   <!--PHOSPHO_ANALYSIS_END-->\n   ```\n\n5. **重要说明**\n   * 系统会自动执行R脚本并在界面上显示结果\n   * 你的回复只需说明正在进行分析\n   * 不要编造或预测结果数据\n   * 不要显示JSON代码块给用户看\n\n6. **示例**\n   用户：查询PDGFRA的磷酸化位点\n   你的回复：\n   我将为您查询PDGFRA基因的磷酸化位点，正在调用R脚本分析真实的GIST磷酸化蛋白质组学数据...\n   \n   <!--PHOSPHO_ANALYSIS_START-->\n   {\"gene\":\"PDGFRA\",\"analyses\":{\"phosphoproteome\":\"phospho_site_query\",\"transcriptome\":\"\",\"proteome\":\"\"}}\n   <!--PHOSPHO_ANALYSIS_END-->" 
      :
          ...
          if (analysisRequest.gene) {
            ...
            if (analysisRequest.analyses && analysisRequest.analyses.phosphoproteome) {
              ...
              const analysisTypeMap = {
                'metastatic_phospho_signature': 'boxplot_TvsN', // 暂时映射到肿瘤vs正常
                'tumor_vs_normal': 'boxplot_TvsN',
                'tumor_vs_normal_differential_phosphorylation': 'boxplot_TvsN', // 新增映射
                'risk_analysis': 'boxplot_Risk',
                'survival_analysis': 'survival',
                'phospho_site_query': 'query',  // 磷酸化位点查询
                'differential_phosphorylation': 'boxplot_TvsN' // 差异磷酸化
              };
